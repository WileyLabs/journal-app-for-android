...
final class PendingRequests implements Runnable {
	...
	void add(@Nonnull RequestRunnable runnable) {
		synchronized (list) {
			BillingImpl.debug("Adding pending request: " + runnable);
			list.add(runnable);
		}
	}
	...
	void cancelAll() {
		synchronized (list) {
			BillingImpl.debug("Cancelling all pending requests");
	...
	void cancelAll(@Nullable Object tag) {
		synchronized (list) {
			BillingImpl.debug("Cancelling all pending requests with tag=" + tag);
			final Iterator<RequestRunnable> iterator = list.iterator();
			while (iterator.hasNext()) {
				final RequestRunnable request = iterator.next();
	...
	void cancel(int requestId) {
		synchronized (list) {
			BillingImpl.debug("Cancelling pending request with id=" + requestId);
			final Iterator<RequestRunnable> iterator = list.iterator();
			while (iterator.hasNext()) {
				final RequestRunnable request = iterator.next();
	...
	RequestRunnable pop() {
		synchronized (list) {
			...
			if (runnable != null) {
				BillingImpl.debug("Removing pending request: " + runnable);
			}
			return runnable;
		}
	}
	...
	public void run() {
		...
		while (runnable != null) {
			BillingImpl.debug("Running pending request: " + runnable);
			if (runnable.run()) {
				remove(runnable);
				runnable = peek();
			} else {
