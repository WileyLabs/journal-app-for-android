...
public class Checkout {
			...
			for (final String product : products.getIds()) {
				requests.isBillingSupported(product, new RequestListener<Object>() {
					...
					public void onSuccess(@Nonnull Object result) {
                        BillingImpl.warning("onSuccess()");
						onBillingSupported(product, true);
					}

					@Override
					public void onError(int response, @Nonnull Exception e) {
                        BillingImpl.warning("onError()");
	...
	public Inventory loadInventory() {
        ...
        BillingImpl.warning("loadInventory()");
    ...
    private final class OnLoadExecutor implements Executor {
        ...
        public void execute(Runnable command) {
            ...
            synchronized (lock) {
                executor = requests != null ? ((BillingImpl.Requests) requests).getDeliveryExecutor() : null;//TODO put getDeliveryExecutor into interface?
            }

            if (executor != null) {
                executor.execute(command);
            } else {
                BillingImpl.error("Trying to deliver result on stopped checkout.");
            }
        }
    }
