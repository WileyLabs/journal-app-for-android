...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    protected int getCountSelected() {
        ...
        Logger.d("SwipeListView", "selected: " + count);
        return count;
    }
    ...
    private void generateAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
        Logger.d("SwipeListView", "swap: " + swap + " - swapRight: " + swapRight + " - position: " + position);
    ...
    /**
     * Close all opened items except pos
     */
    void closeOpenedItemsExcept(int pos) {
        if (opened != null) {
            int start = swipeListView.getFirstVisiblePosition();
            int end = swipeListView.getLastVisiblePosition();
            for (int i = start; i <= end; i++) {
                if (opened.get(i) && pos != i) {
                    closeAnimate(swipeListView.getChildAt(i - start).findViewById(swipeFrontView), i);
                }
            }
        }
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (minFlingVelocity <= velocityX && velocityX <= maxFlingVelocity && velocityY * 2 < velocityX) {
                    ...
                    Logger.d("SwipeListView", "swapRight: " + swapRight + " - swipingRight: " + swipingRight);
...
//                }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (deltaMode > slop && swipeCurrentAction == SwipeListView.SWIPE_ACTION_NONE && velocityY < velocityX) {
                    ...
                    Logger.d("SwipeListView", "deltaX: " + deltaX + " - swipingRight: " + swipingRight);
                    if (opened.get(downPosition)) {
    ...
    public void move(float deltaX) {
        ...
        if (posX > 0 && !swipingRight) {
            Logger.d("SwipeListView", "change to right");
        ...
        if (posX < 0 && swipingRight) {
            Logger.d("SwipeListView", "change to left");
            swipingRight = !swipingRight;
